...
from config import (
    CLASSIFIER_MODEL_PATH, REGRESSOR_MODEL_PATH, CLUSTERING_MODEL_PATH,
    CLASSIFIER_PARAMS, REGRESSOR_PARAMS, N_CLUSTERS, MODEL_DIR
)
...
class PM25Regressor:
    ...
    def _plot_predictions(self, y_test, y_pred):
        ...
        plt.tight_layout()
        plt.savefig(MODEL_DIR / 'regression_predictions.png', dpi=300, bbox_inches='tight')
        plt.close()
        logger.info(f"Prediction plot saved to {MODEL_DIR / 'regression_predictions.png'}")
...
class CityClustering:
    ...
    def evaluate(self, X):
        ...
        for cluster_id in range(N_CLUSTERS):
            cluster_cities = [city for city, label in zip(self.cities, self.labels) 
                             if label == cluster_id]
...
def train_all_models(data_dict):
    ...
    results['classification'] = classifier.evaluate(X_test_cls, y_test_cls, 
