...
class DataDriftDetector:
    ...
    def kolmogorov_smirnov_test(self, feature: str, current_data: pd.DataFrame) -> Tuple[float, bool]:
        """
    ...
    def chi_square_test(self, feature: str, current_data: pd.DataFrame) -> Tuple[float, bool]:
        """
    ...
    def detect_drift(self, current_data: pd.DataFrame, 
        ...
        report = {
            ...
            'total_features': len(numeric_features) + (len(categorical_features) if categorical_features else 0),
            'drifted_features': [],
            'drift_scores': {},
            'summary': {}
        }
        ...
        report['summary'] = {
            ...
            'drift_percentage': (len(report['drifted_features']) / report['total_features']) * 100,
            'overall_drift': len(report['drifted_features']) > 0
        }
        
        self.drift_reports.append(report)
        
        logger.info(f"Drift Detection: {report['summary']['num_drifted']}/{report['total_features']} features drifted")
        
        return report
    
    def calculate_statistics_drift(self, current_data: pd.DataFrame, 
        ...
        for feature in numeric_features:
            if feature in current_data.columns and feature in self.reference_data.columns:
                ...
                std_change = ((curr_stats['std'] - ref_stats['std']) / ref_stats['std']) * 100
    ...
    def get_drift_history(self, filepath: str = "logs/drift_reports.json") -> List[Dict]:
        """Load drift history"""
        try:
            with open(filepath, 'r') as f:
                return json.load(f)
